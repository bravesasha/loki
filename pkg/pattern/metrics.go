...
type ingesterMetrics struct {
	...
	patternSamples         *prometheus.CounterVec
}
...
func newIngesterMetrics(r prometheus.Registerer, metricsNamespace string) *ingesterMetrics {
	return &ingesterMetrics{
		...
		patternSamples: promauto.With(r).NewCounterVec(prometheus.CounterOpts{
			Namespace: metricsNamespace,
			Subsystem: "pattern_ingester",
			Name:      "pattern_samples",
			Help:      "The total number of pattern samples created to write back to Loki.",
		}, []string{"service_name"}),
	}
}
